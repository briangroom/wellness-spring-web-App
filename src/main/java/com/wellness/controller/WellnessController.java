		    ...
		    public String handlepost(@ModelAttribute("email-lookup") UserLogin userLogin, BindingResult bindingResult, ModelMap model) throws ParseException{    	
			    	 ...
			    	 try {
				    		 if(wellnesService.findUser(userLogin.getEmailAddress())== true){
				    			 model.addAttribute("attribs", wellnesService.getUserByEmailId(userLogin.getEmailAddress()));
			...
			public String changeMembershipType(@ModelAttribute("writeBio") UserLogin userLogin, ModelMap model)
				...
				try {
					...
					model.addAttribute("success",
							"Hi " + wellnesService.getUserById(userLogin.getId()).getFname() + " Thanks, Biography saved");
			...
			public List<UserLogin> lookupMembers(String name) {
				JdbcTemplate jdbcTemplate = new JdbcTemplate(dataSource);        	
		       	final String sql  = "SELECT m.* FROM wellness.users m WHERE m.lname like '%"+ name +"%'"
		       						+ " UNION "
		       						+ "SELECT m.* FROM wellness.users m WHERE m.fname like '%"+ name +"%'";
		       	@SuppressWarnings("unchecked")
		   		List<UserLogin> memberList = jdbcTemplate.query(sql, new BeanPropertyRowMapper( UserLogin.class ));						
		   		return memberList;
